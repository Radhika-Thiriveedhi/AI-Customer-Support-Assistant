...
class DocumentAnalyzer:
    def analyze(self,text):
        ...
        return {"characters":len(text),"words":len(words),"sentences":len(sentences),
                "keywords":[{"word":w,"count":c} for w,c in keys],"summary":" ".join(sentences[:3])[:500]}
    def classify(self,text):
        lower=text.lower()
        labels={"billing":["invoice","charge","payment","bill"],"technical":["error","bug","crash","login","not working"],
                "order":["order","delivery","shipment","tracking"],"account":["password","profile","account"]}
